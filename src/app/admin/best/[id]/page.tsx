// src/app/admin/best/[id]/page.tsx
"use client"
import { useState, useEffect } from "react"
import { useRouter } from "next/navigation"

export default function EditBestPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/best/${params.id}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/admin/best/${params.id}`, {
        method: "PUT",
  ...
  const handleDelete = async () => {
    ...
    setLoading(true)
    try {
      await fetch(`/api/admin/best/${params.id}`, { method: "DELETE" })
      router.push("/admin/best")
      router.refresh()
    } catch (error) {
