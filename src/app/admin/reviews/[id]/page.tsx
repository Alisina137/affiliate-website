...
export default function EditReviewPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/admin/reviews/${params.id}`).then(res => res.json())
    ]).then(([productsData, reviewData]) => {
      setProducts(productsData.data || [])
      if (reviewData.data) {
        setFormData({
          title: reviewData.data.title || "",
          slug: reviewData.data.slug || "",
          productId: reviewData.data.productId || "",
          excerpt: reviewData.data.excerpt || "",
          content: reviewData.data.content || "",
          rating: reviewData.data.rating || 5,
          pros: (reviewData.data.pros || []).join(", "),
          cons: (reviewData.data.cons || []).join(", "),
          verdict: reviewData.data.verdict || "",
          bestFor: reviewData.data.bestFor || "",
          status: reviewData.data.status || "DRAFT",
          featured: reviewData.data.featured || false,
        })
      }
  ...
  const handleDelete = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/admin/reviews/${params.id}`, { method: "DELETE" })
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleDelete} disabled={loading} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50">
          Delete
        </button>
      </div>
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select name="productId" value={formData.productId} onChange={handleChange} required className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select name="rating" value={formData.rating} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="5">5 - Excellent</option><option value="4">4 - Very Good</option>
                <option value="3">3 - Good</option><option value="2">2 - Fair</option>
                <option value="1">1 - Poor</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Best For</label>
              <input type="text" name="bestFor" value={formData.bestFor} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="pros" value={formData.pros} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Cons (comma separated)</label>
              <input type="text" name="cons" value={formData.cons} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Verdict</label>
            <textarea name="verdict" value={formData.verdict} onChange={handleChange} rows={2} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          ...
          <div className="flex items-center gap-3 pt-4 border-t">
            <button type="submit" disabled={loading} className="px-6 py-2 bg-[#1a1a2e] text-white rounded-lg hover:bg-[#2d2d44] transition-colors disabled:opacity-50">
              {loading ? "Saving..." : "Update Review"}
            </button>
            <a href="/admin/reviews" className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors">Cancel</a>
          </div>
        </form>
      </div>
    </div>
