// src/app/admin/statistics/[id]/edit/page.tsx
"use client"

import React, { useState, useEffect } from "react"
import { useRouter } from "next/navigation"

export default function EditStatisticPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Failed to update statistics")
      router.push("/admin/statistics")
      router.refresh()
    } catch (error) {
      setError(error instanceof Error ? error.message : "Failed to update statistics")
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async () => {
    if (!confirm("Are you sure you want to delete this statistics page?")) return
    setLoading(true)
    try {
      const response = await fetch(`/api/admin/statistics/${id}`, { method: "DELETE" })
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea name="data" value={formData.data} onChange={handleChange} required rows={4} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Sources (comma separated)</label>
            <input type="text" name="sources" value={formData.sources} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Methodology</label>
            <textarea name="methodology" value={formData.methodology} onChange={handleChange} rows={3} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
