// src/app/admin/comparisons/[id]/page.tsx
"use client"
...
export default function EditComparisonPage({ params }: { params: { id: string } }) {
  ...
  const [formData, setFormData] = useState({
    title: "", slug: "", productA: "", productB: "", excerpt: "", content: "",
    winner: "", winnerExplanation: "", status: "DRAFT", featured: false,
  })

  useEffect(() => {
    Promise.all([
      fetch("/api/products?limit=100").then(res => res.json()),
      fetch(`/api/admin/comparisons/${params.id}`).then(res => res.json())
      ...
      if (comparisonData.data) {
        ...
        setFormData({
          ...
          productA: data.products?.[0]?.productId || "", productB: data.products?.[1]?.productId || "",
          excerpt: data.excerpt || "", content: data.content || "",
          winner: data.winner || "", winnerExplanation: data.winnerExplanation || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/admin/comparisons/${params.id}`, {
        method: "PUT",
  ...
  const handleDelete = async () => {
    ...
    setLoading(true)
    try {
      await fetch(`/api/admin/comparisons/${params.id}`, { method: "DELETE" })
      router.push("/admin/comparisons")
      router.refresh()
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Product A *</label>
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Product B *</label>
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Winner</label>
            <input type="text" name="winner" value={formData.winner} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Winner Explanation</label>
            <textarea name="winnerExplanation" value={formData.winnerExplanation} onChange={handleChange} rows={2} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
